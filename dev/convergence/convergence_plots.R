...
#' @param type Type of plot: "trace", "density", "acf", "energy", "all"
...
plot.bace_convergence <- function(x, 
                                   type = c("trace", "density", "acf", "energy", "all"),
                                   variables = NULL,
                                   ...) {
  ...
  if (type == "all") {
    ...
    n_plots <- 1  # trace plot always
    if (!is.null(x$method_results$energy)) n_plots <- n_plots + 1
    if (!is.null(x$summary_stats)) n_plots <- n_plots + 1
  ...
  for (var in var_cols) {
    ...
    if (!is.null(conv_object$method_results$summary$var_converged)) {
      if (var %in% names(conv_object$method_results$summary$var_converged)) {
        ...
        mtext(ifelse(converged, "✓ Converged", "✗ Not Converged"), 
